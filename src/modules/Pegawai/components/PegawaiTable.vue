        ...
        <tr v-else v-for="(item, index) in data" :key="item.id">
            ...
            <div class="d-flex align-items-center">
              <div class="avatar avatar-sm me-2 avatar-rounded">
                <img :src="item.image" :alt="item.nama"
                  onerror="this.onerror=null; this.src='/src/assets/img/avatar/defaultuser.png'">
              </div>
              <div>
                <div class="lh-1">
                  <span>{{ item.nama }}</span>
          ...
          <td>
            <span :class="['badge', item.status_class]">
              <!-- item.status_label dan item.status_class dari data yang diolah di usePegawaiActions -->
              <i class="ri-check-fill align-middle me-1"></i>{{ item.status_label }}
            </span>
          </td>
          <td class="action-table-data">
            <div class="edit-delete-action">
              <!-- Event @edit akan memanggil openEditModal di Parent -->
              <a href="#" class="me-2 edit-icon p-2" data-bs-toggle="tooltip" title="Edit"
                @click.prevent="$emit('edit', item)">
                <i data-feather="edit" class="feather-edit"></i>
              </a>

              <!-- Event @delete akan memanggil handleDeletePegawai di Parent -->
